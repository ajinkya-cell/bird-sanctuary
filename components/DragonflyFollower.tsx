...
interface DragonflyFollowerProps {
  active: boolean;
}

// Constant flight speed in pixels per second: calm, subtle, and steady
const FLIGHT_SPEED = 230;
const SLOW_DOWN_RADIUS = 35; // px from cursor where it gently decelerates to rest

export default function DragonflyFollower({ active }: DragonflyFollowerProps) {
  ...
  // Detect touch capability to prevent touch jump anomalies
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (dist > 0.5) {
        ...
        // Subtle banking tilt based on direction of flight
        const targetTilt = dist < 8 ? 0 : Math.max(-12, Math.min(12, (dx / dist) * 11));
  ...
  return (
    <AnimatePresence>
      {shouldShow && (
        ...
        >
          {/* Subtle micro-hover flutter and delicate vibration */}
          <motion.div
            animate={
              shouldReduceMotion
                ? {}
            ...
            className="relative w-12 h-8 sm:w-14 sm:h-[37px] md:w-16 md:h-[42px] -translate-x-1/2 -translate-y-1/2 filter drop-shadow-[0_4px_16px_rgba(212,175,55,0.25)] drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]"
          >
            <Image
              src="/dragonfly.png"
              alt="Dragonfly Follower"
              fill
